...
3. [Спецификация инструмента](docs/tool-spec.md) — источник норм: цели/границы и приёмка §12, текущие расширения §13–35, контракт CLI §1–10; PHP/Laravel SDK — §17 и [контракт расширения](docs/php-sdk-contract.md) (п.1–3 реализованы, п.4 выполнен с нейтральным результатом). Сначала уточняй изменяемое требование и проверку, затем меняй реализацию.
