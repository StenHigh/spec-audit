...
3. [Спецификация инструмента](docs/tool-spec.md) — источник норм: цели/границы и приёмка §12, текущие расширения §13–62, контракт CLI §1–10; PHP/Laravel SDK — §17 и [контракт расширения](docs/php-sdk-contract.md) (п.1–3 реализованы, п.4 выполнен с нейтральным результатом). Сначала уточняй изменяемое требование и проверку, затем меняй реализацию.
...
| [Подсказки, advisory о limitations, модель ролей: §62](docs/tool-spec.md#62-подсказки-из-последнего-решения-advisory-о-limitations-выбор-модели-ролей) | `dispatch/<task>/hints.json` — пути из последнего решения по норме; advisory «supported с N limitations»; mapper на дешёвой модели, redteam и хост — на сильной |
