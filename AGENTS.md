...
3. [Спецификация инструмента](docs/tool-spec.md) — источник норм: цели/границы и приёмка §12, текущие расширения §13–63, контракт CLI §1–10; PHP/Laravel SDK — §17 и [контракт расширения](docs/php-sdk-contract.md) (п.1–3 реализованы, п.4 выполнен с нейтральным результатом). Сначала уточняй изменяемое требование и проверку, затем меняй реализацию.
...
| [Ухудшения и предупреждения: §53](docs/tool-spec.md#53-ухудшения-внутри-gap-и-предупреждения-перед-решением) | `delta.worsened[]`/`totals.worsened` на карте; `review … summary.advisories[]` — прошлый contradicts/relevant без тестовых цитат ролей |
| [Перепрогон одной командой: §54](docs/tool-spec.md#54-перепрогон-затронутых-scope-одной-командой-rerun-run_id-config) | `rerun RUN_ID CONFIG...` — инкрементальный run по каждому scope от `baseline_run` или последнего решённого; очередь заданий; пропуски и ошибки по scope |
| [Журнал индекса version 2: §55](docs/tool-spec.md#55-журнал-индекса-version-2-классификация-источников-в-основании-пакета) | commits с `references`/`classified`; перенос файла между `specs` и `references` при тех же байтах — stale; version 1 читается без классификации |
| [Релизная квалификация: §56](docs/tool-spec.md#56-релизная-квалификация-без--short) | полный набор тестов на обеих платформах, Actions по SHA, строгий `update` в квалификации, литеральный вектор `counts` |
| [Установка skill через staging: §57](docs/tool-spec.md#57-установка-skill-через-staging) | свежая копия skill собирается в `.spec-audit/.skill-staging-*` и публикуется одним rename; прерванная установка не оставляет каталога без receipt |
| [Фильтр фактов экспортёра: §58](docs/tool-spec.md#58-фильтр-фактов-экспортёра-по-множеству-files) | `sdk-typed.php` отбрасывает факты с путём цитаты вне `files`; трейт в примере `plain`; сценарий review 2.7 на PHPStan 2 не воспроизводится |
| [Владелец нормы в сводке: §59](docs/tool-spec.md#59-владелец-нормы-в-сводке--первая-нормативная-цитата) | `navigation.section` — первая нормативная цитата; справочный файл норм не считает; `Source`, журнал и `snapshot_id` не меняются |
| [Бенчмарки и кэш источников: §60](docs/tool-spec.md#60-бенчмарки-большого-run-и-кэш-проверенных-источников) | `tool/bench_test.go` — большой run; `sourceCache` читает и хэширует цитируемый файл один раз на команду; `status` 300→63 мс, 793→123 МБ |
| [Контекст роли и `files.txt`: §61](docs/tool-spec.md#61-состав-контекста-роли-и-dispatchfilestxt) | измерение входа роли на пилоте (`files.json` 529 КБ с хэшами — до трети контекста); `dispatch/files.txt` `kind<TAB>path` для роли, промпт велит grep, не читать |
| [Подсказки, advisory о limitations, модель ролей: §62](docs/tool-spec.md#62-подсказки-из-последнего-решения-advisory-о-limitations-выбор-модели-ролей) | advisory «supported с N limitations»; mapper на дешёвой модели, redteam и хост — на сильной; `hints.json` (62.1) отозван после замера без выигрыша |
| [Актуальность по коду: §63](docs/tool-spec.md#63-актуальность-решений-по-коду-на-карте-корпуса-drift) | `overview.scopes[].drift` — план инкремента от решённого run счётчиками; карта: «актуально по коду» / «затронуто N норм»; `totals.drifted`, `stale_scopes` |
