...
- [ ] Task 3: `tool/update.go` — `version`, проверка манифеста и подписи, `update` с атомарной заменой
...
- [ ] Task 4: `tool/skill.go` — `skill install|update` через `os.Root`, receipt, автомат состояний, `--replace`
...
- [ ] Task 5: `scripts/install.sh` и его проверка через `file://`
